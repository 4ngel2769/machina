...
/**
 * Migration script: JSON files → MongoDB (TypeScript version)
...
import ContainerModel from '../lib/models/Container';
import VMModel from '../lib/models/VM';
import QuotaModel from '../lib/models/Quota';
import PricingTemplateModel from '../lib/models/PricingTemplate';
import TokenRequestModel from '../lib/models/TokenRequest';
import UserContractModel from '../lib/models/UserContract';
...
async function migrateUsers() {
  ...
  for (const user of users) {
    try {
      ...
      console.error(`  ❌ Failed to migrate user ${user.username}:`, (error as Error).message);
    }
  }
...
async function migrateContainers() {
  ...
  for (const container of containers) {
    try {
      ...
      console.error(`  ❌ Failed to migrate container ${container.id}:`, (error as Error).message);
    }
  }
...
async function migrateVMs() {
  ...
  for (const vm of vms) {
    try {
      ...
      console.error(`  ❌ Failed to migrate VM ${vm.id}:`, (error as Error).message);
    }
  }
...
async function migrateQuotas() {
  ...
  for (const quota of quotas) {
    try {
      ...
      console.error(`  ❌ Failed to migrate quota for ${quota.username}:`, (error as Error).message);
    }
  }
...
async function migratePricingTemplates() {
  ...
  for (const template of templates) {
    try {
      ...
      console.error(`  ❌ Failed to migrate template ${template.name}:`, (error as Error).message);
    }
  }
...
async function main() {
  ...
  try {
    ...
    await migrateVMs();
    await migratePricingTemplates();
    await migrateTokenRequests();
    await migrateUserContracts();
